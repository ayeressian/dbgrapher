...
export function segmentAngleRadRelativeToXAxis(
  point1: Point,
  point2: Point
): number {
...
export function rotateCord(
  point: Point,
  angle: number,
  relative: Point = { x: 0, y: 0 }
): Point {
  const tx = point.x - relative.x;
  const ty = point.y - relative.y;
  return {
    x: tx * Math.cos(angle) - ty * Math.sin(angle) + relative.x,
    y: tx * Math.sin(angle) + ty * Math.cos(angle) + relative.y,
  };
}
