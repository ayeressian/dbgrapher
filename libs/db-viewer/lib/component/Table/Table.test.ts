...
import { render, RenderResult, fireEvent, cleanup } from '@testing-library/svelte';
...
describe(Table.name, () => {
	...
	describe('when table is being moved', () => {
		...
		beforeEach(async () => {
			schoolSchemaPos = schoolSchema.tables.find((table) => table.name === 'school')!.pos;
		...
		it('should have correct cordinates', () => {
			...
			expect(schoolElem.getAttribute('x')).toEqual(String(schoolSchemaPos.x + moveCord.x - initCord.x));
			expect(schoolElem.getAttribute('y')).toEqual(String(schoolSchemaPos.y + moveCord.y - initCord.y));
		});
	});
});
