...
describe(DBViewer.name, () => {
	...
	describe('scroll view', () => {
		...
		it('viewport should change', () => {
			expect(svgElem.getAttribute('viewBox')).toEqual(`${deltaX} ${deltaY} ${viewWidth} ${viewHeight}`);
		});
	});
	...
	describe('zoom view', () => {
		...
		beforeEach(() => {
			...
			Object.defineProperty(global.SVGGraphicsElement.prototype, 'getScreenCTM', {
				writable: true,
		...
		it.only('viewport should change properly', async () => {
			viewBoxExpectation(cameraXResult, cameraYResult, viewWidthResult, viewHeightResult);
		});
		...
		describe.skip('zoom view second time', () => {
			...
			beforeEach(() => {
				fireEvent.wheel(svgElem, { secondClientX, secondClientY, secondDeltaY, ctrlKey: true });
			});
